...
class DRRAgent:
    ...
    def train(self, max_episode_num, top_k=False, load_model=False):
        ...
        if load_model:
            self.load_model(ROOT_DIR + "/save_weights/actor_50000.h5",
                            ROOT_DIR + "/save_weights/critic_50000.h5")
            ...
            while not done:
                ...
                if self.buffer.crt_idx > 1 or self.buffer.is_full:
                    ...
                    target_qs = self.critic.target_network(
                        [target_next_action, batch_next_states])
                    min_qs = tf.raw_ops.Min(input=tf.concat(
                        [target_qs, qs], axis=1), axis=1, keep_dims=True)  # Double Q method
                ...
                if done:
                    ...
                    if self.use_wandb:
                        wandb.log({'precision': precision, 'total_reward': episode_reward,
                                  'epsilone': self.epsilon, 'q_loss': q_loss/steps, 'mean_action': mean_action/steps})
